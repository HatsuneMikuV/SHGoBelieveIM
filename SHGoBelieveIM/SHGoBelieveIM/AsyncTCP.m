...
-(void)dealloc {
    if (self.IMLogEnable) NSLog(@"async tcp dealloc");
    self.readSource = nil;
...
- (BOOL)synthesizeIPv6:(NSString*)host port:(int)port addr:(struct sockaddr*)addr addrinfo:(struct addrinfo*)info {
    ...
    if (error) {
        if (self.IMLogEnable) NSLog(@"%s", gai_strerror(error));
        return FALSE;
    }
...
-(BOOL)connect:(NSString*)host port:(int)port cb:(ConnectCB)cb {
    ...
    if (!res) {
        if (self.IMLogEnable) NSLog(@"synthesize ipv6 fail");
        return NO;
    }
    ...
    if (r == -1) {
        if (errno != EINPROGRESS) {
            close(sockfd);
            if (self.IMLogEnable) NSLog(@"connect error:%s", strerror(errno));
            return FALSE;
        }
    }
...
-(void)onWrite {
    ...
    if (n < 0) {
        if (self.IMLogEnable) NSLog(@"sock write error:%d", errno);
...
-(void)close {
    ...
    void (^on_cancel)(void) = ^{
        --count;
        if (count == 0) {
            if (self.IMLogEnable) NSLog(@"async tcp closed");
        }
    };
    ...
    if (self.writeSource) {
        if (self.IMLogEnable) NSLog(@"cancel write source");
    ...
    if (self.readSource) {
        if (self.IMLogEnable) NSLog(@"cancel read source");
    ...
    if (self.sock != -1) {
        if (self.IMLogEnable) NSLog(@"close socket");
