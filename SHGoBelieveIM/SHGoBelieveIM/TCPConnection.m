...
-(void)startRechabilityNotifier {
    ...
    self.reach.reachableBlock = ^(GOReachability*reach) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (wself.IMLogEnable) NSLog(@"internet reachable");
            wself.reachable = YES;
            if (wself != nil && !wself.stopped && !wself.isBackground) {
                if (wself.IMLogEnable) NSLog(@"reconnect im service");
                [wself suspend];
                [wself resume];
            }
        });
    ...
    self.reach.unreachableBlock = ^(GOReachability*reach) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (wself.IMLogEnable) NSLog(@"internet unreachable");
            wself.reachable = NO;
...
-(void)enterForeground {
    if (self.IMLogEnable) NSLog(@"im service enter foreground");
    self.isBackground = NO;
...
-(void)enterBackground {
    if (self.IMLogEnable) NSLog(@"im service enter background");
...
-(void)start {
    if (!self.host || !self.port) {
        if (self.IMLogEnable) NSLog(@"should init im server host and port");
        exit(1);
    }
    if (!self.stopped) {
        return;
    }
    if (self.IMLogEnable) NSLog(@"start im service");
    self.stopped = NO;
    if (self.reachable) {
        [self resume];
    }
...
-(void)stop {
    ...
    if (self.IMLogEnable) NSLog(@"stop im service");
    self.stopped = YES;
...
-(void)suspend {
    ...
    if (self.IMLogEnable) NSLog(@"suspend im service");
    self.suspended = YES;
...
-(void)resume {
    ...
    if (self.IMLogEnable) NSLog(@"resume im service");
    self.suspended = NO;
...
-(void)close {
    if (self.tcp) {
        if (self.IMLogEnable) NSLog(@"im service on close");
        [self.tcp flush];
        [self.tcp close];
        self.tcp = nil;
    }
...
-(void)startConnectTimer {
    ...
    if (self.IMLogEnable) NSLog(@"start connect timer:%lld", t/NSEC_PER_SEC);
}
...
-(void)onRead:(NSData*)data error:(int)err {
    if (err) {
        if (self.IMLogEnable) NSLog(@"tcp read err");
        [self handleClose];
        return;
    } else if (!data) {
        if (self.IMLogEnable) NSLog(@"tcp closed");
        [self handleClose];
        return;
    } else {
...
-(NSString*)resolveIP:(NSString*)host {
    ...
    if (s != 0) {
        if (self.IMLogEnable) NSLog(@"get addr info error:%s", gai_strerror(s));
        return nil;
    }
    NSString *ip = nil;
    rp = result;
...
-(void)refreshHostIP {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        if (self.IMLogEnable) NSLog(@"refresh host ip...");
        NSString *ip = [self resolveIP:self.host];
        if (self.IMLogEnable) NSLog(@"host:%@ ip:%@", self.host, ip);
        if ([ip length] > 0) {
            self.hostIP = ip;
...
-(void)connect {
    ...
    if (self.stopped) {
        if (self.IMLogEnable) NSLog(@"opps......");
        return;
    }
    ...
    self.tcp.IMLogEnable = self.IMLogEnable;
    __weak TCPConnection *wself = self;
    BOOL r = [self.tcp connect:self.hostIP port:self.port cb:^(AsyncTCP *tcp, int err) {
        if (err) {
            if (wself.IMLogEnable) NSLog(@"tcp connect err");
            wself.connectFailCount = wself.connectFailCount + 1;
            [wself close];
            wself.connectState = STATE_CONNECTFAIL;
            [wself publishConnectState:STATE_CONNECTFAIL];
            [wself startConnectTimer];
            return;
        } else {
            if (wself.IMLogEnable) NSLog(@"tcp connected");
            wself.connectFailCount = 0;
            wself.connectState = STATE_CONNECTED;
            [wself publishConnectState:STATE_CONNECTED];
    ...
    if (!r) {
        if (self.IMLogEnable) NSLog(@"tcp connect err");
        self.connectFailCount = wself.connectFailCount + 1;
...
-(void)ping {
    if (self.tcp != nil && self.pingTimestamp == 0) {
        if (self.IMLogEnable) NSLog(@"send ping");
        [self sendPing];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (self.pingTimestamp > 0 && now - self.pingTimestamp >= 3) {
                if (self.IMLogEnable) NSLog(@"ping timeout");
                [self handleClose];
            }
        });
    }
